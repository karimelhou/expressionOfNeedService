        ...
        stage('Docker Build') {
            steps {
                script {
                    ...
                    dockerImage = docker.build("expressionofneedservice-app:${env.BUILD_ID}")
                }
            }
        }
        ...
        stage('Push') {
            steps {
                script {
                    ...
                    sh "docker tag expressionofneedservice-app:${env.BUILD_ID} karimelhou/expressionofneedservice:${env.BUILD_ID}"
                    // Pushing the image to Docker Hub
                    sh "docker push karimelhou/expressionofneedservice:${env.BUILD_ID}"
                }
            }
        }
        ...
        stage('Docker Run') {
            steps {
                script {
                    ...
                    sh "docker run -d -p 8382:8382 expressionofneedservice-app:${env.BUILD_ID}"
                }
            }
        }
